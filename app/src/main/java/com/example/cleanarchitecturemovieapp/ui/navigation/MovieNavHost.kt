...
import com.example.cleanarchitecturemovieapp.ui.screens.detail.DetailScreen
import com.example.cleanarchitecturemovieapp.ui.screens.favorite.FavoriteScreen
import com.example.cleanarchitecturemovieapp.ui.screens.home.HomeScreen

@Composable
fun MovieNavHost(navController: NavHostController, innerPadding: PaddingValues) {
    NavHost(
        navController = navController,
        startDestination = MovieScreen.HOME.route,
...
fun NavGraphBuilder.detailComposable(navController: NavHostController) {
    composable(
        route = "${MovieScreen.DETAIL.route}/{movieId}",
        arguments = listOf(
...
fun NavGraphBuilder.favoriteComposable(navController: NavHostController) {
    composable(
        route = MovieScreen.FAVORITE.route,
        enterTransition = {
            return@composable slideIntoContainer(
                AnimatedContentTransitionScope.SlideDirection.Start, tween(500)
            )
